...
pub fn draw(app: &mut MusicApp, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if ui.small_button("＋ 添加文件").on_hover_text("添加音频文件").clicked() {
            ...
            if ui.small_button("📁 添加文件夹").on_hover_text("递归添加文件夹内音频").clicked() {
                if let Some(dir) = rfd::FileDialog::new().pick_folder() {
                    app.add_paths(vec![dir]);
                }
            }
            ui.separator();
            if ui.small_button("打开列表").on_hover_text("加载 m3u/m3u8 播放列表").clicked() {
            ...
            if ui.small_button("保存列表").on_hover_text("保存为 m3u8").clicked() {
                if let Some(path) = rfd::FileDialog::new()
